...
func Watch(filePath string, p parser.LogParser, every time.Duration) (LogScanner, error) {
	log := zap.NewExample().Sugar()
	defer log.Sync()
	f, err := os.OpenFile(filePath, os.O_RDONLY, 0755)
	if err != nil {
		return nil, errors.Wrapf(err, "failed to read from path %s", filePath)
	}
	ls := logScanner{
		SugaredLogger: log,
		interval:      every,
...
func (ls *logScanner) readLines(f *os.File) ([]parser.Log, error) {
	newLogs := []parser.Log{}
...
func (ls *logScanner) hasChanged(f *os.File, stats os.FileInfo) (int64, bool, error) {
...
func (ls *logScanner) loop(f *os.File) {
